...
ax1.text(100, ax1.get_ylim()[0]+0.6, "Stage 1: MSE", ha="center", fontsize=9, color="0.35")
ax1.text(250, ax1.get_ylim()[0]+0.6, "Stage 2: composite loss", ha="center", fontsize=9, color="#2a6f9e")
ax1.set_ylabel("validation PSNR (dB)"); ax1.set_xlim(0, 300)
ax1.legend(loc="lower right", frameon=False, fontsize=9)
ax1.set_title("")

# ---- Bottom: Stage-2 zoom ----
ax2.axhline(A_final, color=CB, ls=":", lw=1.4)
ax2.text(299, A_final+0.015, f"Arm A converged ({A_final:.2f} dB val)", ha="right", va="bottom", fontsize=8.5, color=CB)
ax2.plot(epR, vR, color=CR, lw=2.0, marker="o", ms=3.2, label="Arm R (L1 base): stays flat")
ax2.plot(epS, vS, color=CS, lw=2.0, marker="s", ms=3.2, label="Arm S (NLL base): dips, then recovers")
# annotate final values + the reported test PSNR
ax2.annotate(f"R  val {vR[-1]:.2f}  |  test 34.545 dB", (epR[-1], vR[-1]), xytext=(-6, 8),
             textcoords="offset points", ha="right", color=CR, fontsize=9, fontweight="bold")
ax2.annotate(f"S  val {vS[-1]:.2f}  |  test 34.608 dB", (epS[-1], vS[-1]), xytext=(-6, -14),
             textcoords="offset points", ha="right", color=CS, fontsize=9, fontweight="bold")
...
ax2.set_ylim(lo, max(A_final, np.nanmax(vR), np.nanmax(vS)) + 0.25)
