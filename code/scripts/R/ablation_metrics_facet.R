...
csv <- getarg("--csv","results/ablation_facet_data.csv"); out <- getarg("--out","ablation_metrics_facet.pdf")
...
famv <- c(arm_a_l2="Pixel-norm",arm_i_l1="Pixel-norm",arm_j_l1_ssim_ffl="Pixel-norm",arm_q_charb="Pixel-norm",
 ...
 nafnet="SOTA",scunet="SOTA",sharpxr="SOTA")
...
long <- long |> mutate(Arm=factor(lab[arm], levels=lab[ord]),
                       Family=factor(famv[arm], levels=c("Pixel-norm","NLL","VAE","Fine-tune","Baseline","SOTA","Failure")),
...
p <- ggplot(long, aes(m, Arm, colour=Family)) +
  ...
  scale_colour_manual(values=c("Pixel-norm"="#4c72b0","NLL"="#dd8452","VAE"="#8172b3",
                               "Fine-tune"="#937860","Baseline"="#55a868","SOTA"="#e377c2","Failure"="#c44e52")) +
